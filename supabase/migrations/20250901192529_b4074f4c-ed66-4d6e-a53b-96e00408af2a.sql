-- Fix database function conflict by dropping the duplicate function
-- Keep only the single-parameter version that the UI expects

DROP FUNCTION IF EXISTS public.calculate_performance_metrics(p_user_id uuid, p_course_id uuid);

-- Ensure the single-parameter version exists and handles edge cases properly
CREATE OR REPLACE FUNCTION public.calculate_performance_metrics(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
    ...
    LOOP
        ...
        -- Insert performance metrics only if we have meaningful data
        IF avg_grade > 0 OR completion_rate > 0 OR total_study_hours > 0 THEN
    ...
    -- Calculate overall metrics only if user has courses
    IF EXISTS (SELECT 1 FROM public.courses WHERE user_id = p_user_id AND is_active = true) THEN
        ...
        -- Insert overall metrics only if meaningful
        IF avg_grade > 0 OR completion_rate > 0 OR study_consistency > 0 OR total_study_hours > 0 THEN
            INSERT INTO public.performance_analytics (
                user_id, course_id, metric_type, metric_value, time_period, calculation_date, metadata
            ) VALUES 
