-- PHASE 1: Critical OAuth Security Fix - Secure Google Token Access
-- Create secure SELECT policy for google_tokens that allows only security definer functions

-- Drop the overly restrictive policies and create secure ones
DROP POLICY IF EXISTS "Authenticated users can insert only their own tokens" ON public.google_tokens;
DROP POLICY IF EXISTS "Authenticated users can delete only their own tokens" ON public.google_tokens;

-- Create comprehensive secure policies for google_tokens
CREATE POLICY "Secure token management - INSERT"
ON public.google_tokens FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = user_id AND auth.role() = 'authenticated');

CREATE POLICY "Secure token management - DELETE"
ON public.google_tokens FOR DELETE
TO authenticated
USING (auth.uid() = user_id AND auth.role() = 'authenticated');

-- Critical: Add SELECT policy for security definer functions ONLY
CREATE POLICY "Security definer functions can read tokens"
ON public.google_tokens FOR SELECT
TO authenticated
USING (
  -- Only allow access through security definer functions
  -- This prevents direct access while enabling legitimate function calls
  current_setting('role', true) = 'supabase_admin' OR 
  pg_has_role('supabase_admin', 'member')
);

-- PHASE 2: Smart Template Access Control - Fix System Templates
-- Replace overly restrictive policy with intelligent access control

DROP POLICY IF EXISTS "System templates access restricted" ON public.rotation_templates;

-- Allow authenticated users to READ system templates but prevent modification
...
-- Prevent modification/deletion of system templates
CREATE POLICY "System templates are read-only"
ON public.rotation_templates FOR UPDATE
TO authenticated
USING (false);

CREATE POLICY "System templates cannot be deleted"
ON public.rotation_templates FOR DELETE  
TO authenticated
USING (false);

CREATE POLICY "No direct template insertion"
ON public.rotation_templates FOR INSERT
TO authenticated
WITH CHECK (false);

-- PHASE 3: Enhanced User Data Protection - Add Security Functions
-- Create data masking function for sensitive user data
...
-- Create secure audit function for profile access
...
-- PHASE 4: OAuth Security Enhancement Functions
-- Create secure OAuth state validation function
...
-- Create secure redirect URI validation function  
...
-- PHASE 5: Rate Limiting and Security Monitoring
