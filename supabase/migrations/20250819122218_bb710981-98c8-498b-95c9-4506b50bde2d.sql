...
BEGIN
    ...
    SELECT COUNT(*) INTO assignment_count FROM assignments WHERE user_id = p_user_id;
    SELECT COUNT(*) INTO completed_assignments FROM assignments WHERE user_id = p_user_id AND is_completed = true;
    ...
    (p_user_id, NULL, 'total_study_hours', total_study_hours, 'all_time', CURRENT_DATE,
     jsonb_build_object('sessions_count', (SELECT COUNT(*) FROM study_sessions WHERE user_id = p_user_id AND status = 'completed')));
